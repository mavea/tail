	...
	return MainList{
		...
		func(val []uint64) (Command, bool) {
			...
			for i := 0; i < len(val); i++ {
				...
				switch val[i+1] {
				case 5:
					if i+2 >= len(val) || val[i+2] > 255 {
						return nil, false
					}
					i += 2
				case 2:
					if i+4 >= len(val) || val[i+2] > 255 || val[i+3] > 255 || val[i+4] > 255 {
						return nil, false
					}
					i += 4
				default:
		...
		func(val []uint64) (Command, bool) {
			...
			return &commandUint{key: KeyNewLine,
				vals: val,
			}, true
		}, //16
	}
...
func (o OtherList) GetFunction(_ rune) (func([]uint64) (Command, bool), bool) {
	return nil, false
}
