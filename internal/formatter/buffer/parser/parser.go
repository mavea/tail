...
func (par *parser) getNewLineCommand() Command {
	if f, b := par.ansiMainList.GetFunction(1); b {
...
func (par *parser) enterInParseANSIEscapeSequences(strRunes []rune) (int, Command) {
	...
	case ']':
		...
		case strRunes[i] == '?':
			...
			if f, b := par.ansiWindowAndPalettes.GetFunction('?'); b {
				if command, ok = f(sl, ""); !ok {
					return 1, nil
				}
				return i + 1, command
		...
		case len(strRunes) > i+3 && string(strRunes[i:i+3]) == "rgb":
			...
			i += offset2
			if f, b := par.ansiWindowAndPalettes.GetFunction('s'); b {
		...
		default:
			...
			if f, b := par.ansiWindowAndPalettes.GetFunction('t'); b {
